...
/**
 * HomePage.tsx
 * - Seluruh logic dari app/page.tsx kamu dipindah ke sini agar tidak ada kode nyasar di top-level.
 * - Sudah aman untuk Next.js App Router (server file tipis, client file penuh).
 * - Komponen ini mempertahankan UI & behaviour sebelumnya (Earn, Missions, Activity, Profile, Leaderboard).
 */
...
// ==== (Opsional Mini Dapp SDK shim — aman walau belum aktif) ====
import { getMiniDapp } from "@/lib/miniDapp";
...
export default function HomePage() {
  ...
  const [lockedPositions, setLockedPositions] = useState<{ plan: string; amount: number; start: number }[]>([]);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      // Kontrak read-only
      const usdtRead: any = new Contract(USDT, (usdtJson as any).abi ?? usdtJson, readProvider);
      const vaultRead: any = new Contract(VAULT, (vaultJson as any).abi ?? vaultJson, readProvider);
  ...
  const fetchActivity = useCallback(async () => {
    try {
      ...
      const vaultIface = new Contract(VAULT, (vaultJson as any).abi ?? vaultJson).interface as any;

      // ethers v6 event topics
